...
export function LabSidebar({ isMobileOpen, onClose }) {
  ...
  const { labCases, activeCaseId } = useLabStore();
  ...
  const menuItems = [
    ...
    { key: 'crown', label: 'Crown Tracking', icon: 'Layers', path: '/lab/crown' },
    { key: 'implant', label: 'Implant Cases', icon: 'Wrench', path: '/lab/implant' },
          ...
          <div className="p-4 mx-4 my-3 rounded-lg bg-muted/50 border border-border/40 text-left">
            ...
            {activeCase && (
              <div className="mt-2 pt-2 border-t border-border/40 text-[10px] text-muted-foreground flex items-center gap-1.5">
                ...
                <span>Case: <strong className="text-foreground">{activeCase.patientName} ({activeCase.type})</strong></span>
              </div>
            )}
          </div>
        ) : (
